...
export class InputRealisasiService {
  ...
  async getMyDecisions(user: User, periodId?: string) {
    ...
    for (const r of records) {
      const history = Array.isArray(r.history)
        ...
        : [];
      const myActions = history.filter(
        (h) =>
          h["actorId"] === user.id ||
          (h["actorId"] == null && h["actor"] === user.name),
      );
      ...
      result.push({
        ...
        myActions: myActions.map((h) => ({
          ...
          ts: String(h["ts"] ?? ""),
          stepIndex: Number(h["stepIndex"] ?? 0),
        })),
      });
    }
    return result;
  ...
  async getReviewerCandidates(unitCode?: string, bidang?: string) {
    ...
    return {
      checkers: users.filter(
        (u) =>
          CHECKER_ROLES.includes(u.role) &&
  ...
  async submit(
    ...
    if (!period)
      throw new BadRequestException(
        periodId ? "Periode tidak ditemukan" : "Tidak ada periode aktif",
      );
    ...
    if (!win.isOpen) {
      ...
      throw new ForbiddenException(
        `Pengisian realisasi periode ${period.label} hanya dibuka ${fmt(win.start)} s.d. ${fmt(win.end)}. ` +
          (win.daysUntilOpen > 0
            ? `Window akan dibuka dalam ${win.daysUntilOpen} hari.`
            : "Window pengisian telah berakhir."),
      );
    }

    const existing = await this.prisma.inputRealisasi.findUnique({
      where: {
        periodId_unitCode_bidang: { periodId: period.id, unitCode, bidang },
      },
    });
    const baseHistory =
    ...
    const { targetByMaster } = await this.periodTargetSvc.resolveForPackage(
      period.id,
      unitCode,
      bidang,
      items,
    ...
    const result = await this.prisma.inputRealisasi.upsert({
      ...
      update: {
        ...
        selfAssessment: selfAssessment as object,
        submitter: user.name,
        submitterId: user.id,
        status: "submitted",
    ...
    await this.prisma.auditLog.create({
      data: {
        actor: user.name,
        userId: user.id,
        action: "realisasi.submit",
        entity: "InputRealisasi",
        targetId: result.id,
      },
    });
  ...
  async delete(user: User, id: string) {
    ...
    if (r.status === "approved")
      throw new ForbiddenException(
        "Realisasi yang sudah disetujui final tidak dapat dihapus",
      );
    const isOwner = r.submitterId && r.submitterId === user.id;
    if (!isOwner && user.role !== Role.GM) {
      throw new ForbiddenException(
        "Hanya pengirim atau GM yang dapat menghapus realisasi ini",
      );
    }
  ...
  async updateValues(
    ...
    if (r.status !== "submitted")
      throw new ForbiddenException(
        "Hanya realisasi yang sedang direview yang dapat diedit",
      );
    const steps = (r.steps as unknown as Step[]) ?? [];
    ...
    if (JSON.stringify(oldValues) !== JSON.stringify(values)) {
      await this.prisma.revisionLog.create({
        data: {
          entity: "input_realisasi",
          targetId: id,
          periodId: r.periodId,
          actor: user.name,
          actorId: user.id,
          field: "values",
          oldValue: oldValues as object,
          newValue: values as object,
          note,
        },
      });
    }
  ...
  async getRevisionHistory(id: string) {
    ...
    const items = Object.values(
      (r.values ?? {}) as Record<string, Record<string, unknown>>,
    );
    const periodTargetIds =
      await this.periodTargetSvc.findPeriodTargetIdsForPackage(
        r.periodId,
        r.unitCode,
        r.bidang,
        items,
    ...
    const [ownRevisions, targetRevisions] = await Promise.all([
      this.prisma.revisionLog.findMany({
        where: { entity: "input_realisasi", targetId: id },
        orderBy: { createdAt: "desc" },
      }),
      periodTargetIds.length > 0
        ? this.prisma.revisionLog.findMany({
            where: {
              entity: "period_target",
              targetId: { in: periodTargetIds },
            },
            orderBy: { createdAt: "desc" },
          })
        : Promise.resolve([]),
    ...
    return [...ownRevisions, ...targetRevisions].sort(
  ...
  async addEvidence(user: User, id: string, files: UploadFile[]) {
    if (!files?.length)
      throw new BadRequestException("Tidak ada berkas yang diunggah");
    const r = await this.prisma.inputRealisasi.findUnique({ where: { id } });
    if (!r) throw new NotFoundException("Realisasi tidak ditemukan");
    const steps = (r.steps as unknown as Step[]) ?? [];
    const allowed =
      r.submitterId === user.id ||
      user.role === Role.GM ||
      stepMatches(steps[r.currentStepIndex], user);
    if (!allowed)
      throw new ForbiddenException(
        "Anda tidak berwenang melampirkan evidence pada realisasi ini",
      );

    const existing: Attachment[] = Array.isArray(r.attachments)
      ? (r.attachments as unknown as Attachment[])
      : [];
    if (existing.length + files.length > EVIDENCE_MAX_FILES) {
      throw new BadRequestException(
        `Maksimal ${EVIDENCE_MAX_FILES} berkas per realisasi`,
      );
    }
    for (const f of files) {
      const ext = path.extname(f.originalname).toLowerCase();
      if (!EVIDENCE_EXT.includes(ext))
        throw new BadRequestException(
          `Tipe berkas tidak didukung: ${f.originalname}. Hanya PDF, Excel, Word, JPG/PNG.`,
        );
      if (f.size > EVIDENCE_MAX_BYTES)
        throw new BadRequestException(
          `Berkas ${f.originalname} melebihi 10 MB`,
        );
    }
    ...
    await this.prisma.inputRealisasi.update({
      where: { id },
      data: { attachments: attachments as object },
    });
    await this.prisma.auditLog.create({
      data: {
        actor: user.name,
        userId: user.id,
        action: "realisasi.evidence.add",
        entity: "InputRealisasi",
        targetId: id,
        note: added.map((a) => a.name).join(", "),
      },
    });
    return attachments;
  }
  ...
  async getEvidenceFile(id: string, fileId: string) {
    ...
    if (!fs.existsSync(fp))
      throw new NotFoundException("Berkas hilang dari penyimpanan");
  ...
  async deleteEvidence(user: User, id: string, fileId: string) {
    const r = await this.prisma.inputRealisasi.findUnique({ where: { id } });
    if (!r) throw new NotFoundException("Realisasi tidak ditemukan");
    const list: Attachment[] = Array.isArray(r.attachments)
      ? (r.attachments as unknown as Attachment[])
      : [];
    const att = list.find((a) => a.id === fileId);
    if (!att) throw new NotFoundException("Berkas tidak ditemukan");
    if (r.submitterId !== user.id && user.role !== Role.GM)
      throw new ForbiddenException(
        "Hanya pengunggah/pengirim atau GM yang dapat menghapus evidence",
      );
    try {
      fs.unlinkSync(path.join(EVIDENCE_DIR, id, att.file));
    } catch {
      /* abaikan */
    }
  ...
  async review(
    ...
    if (r.status !== "submitted")
      throw new ForbiddenException(
        "Realisasi tidak dalam status menunggu review",
      );
    if (!note?.trim())
      throw new BadRequestException(
    ...
    if (!stepMatches(steps[curIdx], user)) {
      throw new ForbiddenException(
        `Realisasi ini menunggu langkah "${steps[curIdx]?.label ?? "lain"}", bukan langkah Anda`,
      );
    }
    const baseHistory = Array.isArray(r.history) ? (r.history as object[]) : [];
      ...
      if (picRen.length) {
        await this.prisma.notification.createMany({
          data: picRen.map((u) => ({
            userId: u.id,
            type: "alert",
            title: "KM Sementara Perlu Dikoreksi",
    ...
    if (action === "reject") {
      ...
      await this.prisma.auditLog.create({
        data: {
          actor: user.name,
          userId: user.id,
          action: toPrev ? "realisasi.return_step" : "realisasi.reject",
          entity: "InputRealisasi",
          targetId: id,
          note,
        },
  ...
  async resolveTargetFix(
    ...
    const r = await this.prisma.inputRealisasi.findUnique({ where: { id } });
    if (!r) throw new NotFoundException("Realisasi tidak ditemukan");
    if (r.status !== "target_fix")
      throw new ForbiddenException(
        "Realisasi ini tidak sedang menunggu koreksi target",
      );
    if (!this.periodTargetSvc.isPicRen(user))
      throw new ForbiddenException(
        "Hanya PIC REN yang dapat menyelesaikan koreksi target",
      );
    if (!Array.isArray(updates) || updates.length === 0)
      throw new BadRequestException("Minimal satu koreksi target diperlukan");
    if (!note?.trim())
      throw new BadRequestException("Catatan koreksi wajib diisi");

    for (const u of updates) {
      await this.periodTargetSvc.updateTarget(
        user,
        r.periodId,
        u.kpiAssignmentId,
        u.target,
    ...
    if (r.submitterId) {
      await this.prisma.notification.create({
        data: {
          userId: r.submitterId,
          type: "alert",
          title: "Target KM Sementara Dikoreksi — Perlu Resubmit",
  ...
  async getBundle(periodId?: string) {
    ...
    const components = await this.prisma.inputRealisasi.findMany({
      where: {
        periodId: period.id,
        status: { in: ["submitted", "target_fix", "ready", "approved"] },
      },
      orderBy: [{ unitCode: "asc" }, { bidang: "asc" }],
    });
    const bundle = await this.prisma.realisasiBundle.findUnique({
      where: { periodId: period.id },
    });
    ...
    const inProgressCount = components.filter(
      (c) => c.status === "submitted" || c.status === "target_fix",
    ).length;
    ...
    return {
      ...
      components: components.map((c) => {
        const values = (
          c.values && typeof c.values === "object" ? c.values : {}
        ) as Record<string, Record<string, unknown>>;
        ...
        for (const [k, v] of Object.entries(values)) {
          enriched[k] = {
            ...
            persenAgregasi: persenLookup.get(
              `${v?.["masterKpiId"]}|${c.unitCode}|${c.bidang}`,
            ),
          };
        }
        return {
  ...
  async reviewBundle(
    ...
    if (user.role !== Role.GM)
      throw new ForbiddenException(
        "Hanya General Manager yang menyetujui bundle realisasi",
      );
    if (!note?.trim())
    ...
    const components = await this.prisma.inputRealisasi.findMany({
      where: {
        periodId: period.id,
        status: { in: ["submitted", "target_fix", "ready"] },
      },
    });
    if (components.length === 0)
      throw new BadRequestException(
        "Tidak ada realisasi yang siap dikonsolidasi pada periode ini",
      );

    if (action === "approve") {
      if (!components.every((c) => c.status === "ready")) {
        throw new ForbiddenException(
          "Masih ada komponen yang belum lolos sampai SM Perencanaan & PC (termasuk yang menunggu koreksi target)",
        );
      }
    ...
    if (submitterIds.length) {
      await this.prisma.notification.createMany({
        data: submitterIds.map((sid) => ({
          ...
          title:
            action === "approve"
              ? "Bundle Realisasi Disetujui GM"
              : "Bundle Realisasi Dikembalikan GM",
          msg:
            action === "approve"
          ...
          route: action === "approve" ? "/operational" : "/input-realisasi",
          unread: true,
        })),
      });
    }
    await this.prisma.auditLog.create({
      data: {
        actor: user.name,
        userId: user.id,
        action: `realisasi.bundle.${action}`,
        entity: "RealisasiBundle",
        targetId: bundle.id,
        note,
      },
    });
    ...
    if (action === "approve") {
      ...
      setImmediate(() => {
        Promise.all([
          ...
          console.error(
            "[Snapshot Refresh] Error saat memperbarui snapshot:",
            err,
          );
        });
      });
